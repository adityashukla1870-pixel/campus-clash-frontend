    ...
    <>
      ...
      <div style={pageStyle}>
        <div style={innerStyle}>
          <div style={{cursor:'pointer',color:'var(--text-secondary)',fontSize:14,marginBottom:24}} onClick={() => navigate(-1)}>← Back</div>
          <h1 style={{fontFamily:'var(--font-display)',fontSize:28,fontWeight:700,marginBottom:6}}>🏆 {data.tournament_name}</h1>
              ...
              {data.bracket.rounds.map((round, ridx) => (
                <div key={ridx} style={{minWidth:260, flexShrink:0}}>
                  <div style={{fontFamily:'var(--font-display)',fontWeight:700,fontSize:16,marginBottom:14,color:'var(--purple-light)'}}>
                    {ridx === data.bracket.rounds.length - 1 ? "🏁 Final" : `Round ${ridx + 1}`}
                  </div>
                  {round.map((match) => (
